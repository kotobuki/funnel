/****
* Magnetometer Calibration for fio and LSM303DLH
* written by takanori endo (2012/8/22)
* 
****/
...
void draw() {
  ...
  pushMatrix(); {
    ...
    if (magnetomIndex < NUM_MAGN_SAMPLES ) {
      double[] d = compass.m.getRawData();
      //double[] d = compass.m.getCompensateData();

        magnetom[magnetomIndex][0] = (float)d[0];  // x
        magnetom[magnetomIndex][1] = (float)d[1];  // y
        magnetom[magnetomIndex][2] = (float)d[2];  // z
