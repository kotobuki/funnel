...
On Windows, using X-CTU will be an alternative way to configure XBee modems. Write \texttt{coordinator\_auto\_reset.pro} or \texttt{coordinator.pro} (if you don't want to enable the auto reset function) in \texttt{hardware/fio/xbee} to a coordinator, and \texttt{router.pro} to end devices (i.e. XBee modems for FIO boards) using X-CTU. Please change parameters if needed.
...
If you can't see a string like `\texttt{Firmata Protocol Version: 2.1}', the firmware might have failed to upload. Please refer the section \ref{uploading_firmata_arduino} to try again. If you see any error messages, please make sure that:
...
\item An appropriate serial port (e.g. COM3) is specified (if needed)
\item An appropriate baud rate (e.g. 57600) is specified
\end{itemize}
...
If you can't see a string like `\texttt{Firmata Protocol Version: 2.1}', the firmware might have failed to upload. Please refer the section \ref{uploading_firmata_fio} to try again. If you see any error messages, please make sure that:
