...
Funnel v1.0は以下のI/Oモジュール（一部はマイコンモジュール）への対応を予定しています。新規に開発するFunnel I/Oモジュール以外に、既に普及しているGainer\footnote{2007年5月の時点で500台以上。http://gainer.cc/}およびArduinoシリーズ\footnote{2007年5月の時点でシリーズ合計で10000台以上。ファームウェアはFirmataを使用。http://arduino.cc/}、およびMaxStream社のXBeeシリーズに対応します。これにより、Funnelはv1.0のリリース時点で1万数千台以上のハードウェア環境で動作できることになります。
\begin{itemize}
\item Funnel I/Oモジュール（USB接続のドングル＋無線モジュール）
\item Gainer I/Oモジュール（USB接続）
\item Arduino（USB接続）
\item MaxStream XBeeシリーズ1/2（USB接続のドングル＋無線モジュール）
...
\begin{itemize}
\item USB⇔XBeeドングル（New Micros製USB XBee Dongleなど）
	\begin{itemize}
	\item USB-to-UART変換ブリッジ（FTDI製FT232RL）
	\item ワイヤレス通信モジュール（MaxStream製XBee）
	\end{itemize}
%\item 追加モジュール（有線）
%	\begin{itemize}
...
\item 無線モジュール
	\begin{itemize}
	\item PSoCマイコン（Cypress Semiconductor製CY8C27143）
	\item ワイヤレス通信モジュール（MaxStream製XBee）
	\end{itemize}
\end{itemize}
...
USB⇔XBeeドングルはUSBとIEEE 802.15.4の間の変換を行います。無線モジュールのハードウェアとファームウェアは基本となるものを開発し、リファレンスデザインとして公開しますが、PSoC Express\footnote{Cやアセンブラでコードを書くことなくマイコンのファームウェアを開発できるツール。}やPSoC Designer\footnote{PSoCマイコンの全ての機能を利用できる開発ツール。利用できる言語はCまたはアセンブラ。}により、ユーザ自身がデザインして簡単に追加することも可能です。
...
IOSystemクラスは複数のI/Oモジュールから構成されるシステムを表現するためのクラスです。Gainer I/OモジュールやArduino I/Oボードを利用する際には、IOSystemオブジェクトをラップするユーティリティークラス（GainerまたはArduino）を利用してアクセスすることにより、内部構造を意識することなくアクセスできます。

\subsubsection{コンストラクタ}
\texttt{IOSystem(hostName:String, portNumber:int, modules:int, samplingInterval:int)}
