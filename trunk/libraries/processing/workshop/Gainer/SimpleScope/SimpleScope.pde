...
{ 
  ...
  for (int channel = 0; channel < kChannles; channel ++) {
    ...
    text("val: " + gainer.analogInput(channel).value, kLeft + kPoints + 8, kTop + 8 + offset);
    text("max: " + gainer.analogInput(channel).maximum, kLeft + kPoints + 8, kTop + 20 + offset);
    text("min: " + gainer.analogInput(channel).minimum, kLeft + kPoints + 8, kTop + 32 + offset);
    text("avg: " + gainer.analogInput(channel).average, kLeft + kPoints + 8, kTop + 44 + offset);

    values[channel][index] = gainer.analogInput(channel).value;
