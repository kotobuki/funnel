...
\hspace{-8.5pt}{\Large \textsf{簡易マニュアル（ビルド008用）／開発者：小林茂＋遠藤孝則＋増田一太郎}}
...
\item ソフトウェア
	...
	\item Flash Professional CS3・Flex Builder 3・Flex 2/3 SDK\footnote{http://www.adobe.com/jp/products/flex/sdk/}
...
\section{ドライバのインストール}
Gainer I/Oモジュール、Arduino I/Oボード、およびXBeeやFioとの接続に使用するUSB⇔XBee変換モジュール\footnote{SparkFun ElectronicsのXBee Explorer USB（パーツナンバーWRL-08687）など}のほとんどはFTDI社のUSB-to-UART変換ブリッジチップFT232Rシリーズを搭載しています。このため、最初にこのドライバをセットアップしておくことでほとんどの全てのハードウェアを利用できます\footnote{Arduinoを使用する場合には配布パッケージのdriversフォルダ（例：\texttt{arduino-0012/drivers}）にも含まれています。}。
...
Funnel ServerはデフォルトでGainer I/Oモジュールを使用する設定になっています。このため、Gainer I/Oモジュール以外のI/Oモジュールを使用する場合には、ハードウェアに合わせてFunnel Serverの設定ファイル（settings.txt）を変更して下さい\footnote{Windowsの場合、ノートパッドでファイルを開くと改行コードの関係で全てが1行に表示されてしまうかもしれません。ワードパッドであれば問題なく編集できます。}。サンプルとして以下の設定ファイルも配布パッケージに含まれています。
\begin{itemize}
\item settings.arduino.txt：Arduino I/Oボード用の設定ファイル
\item settings.fio.txt：Fio用の設定ファイル
\item settings.gainer.txt：Gainer I/Oモジュール用の設定ファイル
\item settings.xbee.txt：XBee用の設定ファイル
\end{itemize}
