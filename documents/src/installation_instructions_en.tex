...
On Windows, write \texttt{coordinator\_auto\_reset.pro} or \texttt{coordinator.pro} (if you don't want to enable the auto reset function) in \texttt{hardware/fio/xbee} to a coordinator, and \texttt{router.pro} to end devices (i.e. XBee modems for FIO modules) using X-CTU. Please change parameters if needed. On Mac OS X, configure XBee modems manually using XBee Terminal references from table 3.
