...
\texttt{port(n).number} & int & ポートの番号 \\
...
\texttt{port(n).value = } & float & ポートnの値を設定（ポートのdirectionが\texttt{OUTPUT}の場合） \\
\texttt{port(n).lastValue} & float & ポートnの変化する前の値 \\\hline
...
\texttt{port(n).filters} & Array & 現在設定されているフィルタ \\
\texttt{port(n).filters =} & Array & フィルタを設定 \\
...
\subsubsection{イベントハンドラ（Processing）}
\begin{tabular}{lll}
メソッド名 & 戻り値 & 説明 \\\hline
\texttt{risingEdge(PortEvent event)} & void & \texttt{PortEvent.RISING\_EDGE}のハンドラ \\
\texttt{rfallingEdge(PortEvent event)} & void & \texttt{PortEvent.FALLING\_EDGE}のハンドラ \\
\texttt{change(PortEvent event)} & void & \texttt{PortEvent.CHANGE}のハンドラ \\
\end{tabular}

\subsubsection{PortEventの定数}
\begin{tabular}{lll}
イベント名 & 設定先 & 説明 \\\hline
...
fio.port(0).filters = [new Osc(Osc.SQUARE, 2, 0)]; // 波形、周波数、回数（0は無限回）
