...
gio.button.addEventListener(RISING_EDGE, buttonPressed);
...
\item 基本モジュール
	\begin{itemize}
	\item USB-to-UART変換ブリッジ（FT232RL）
...
\subsection{Systemクラス}
Systemクラスは複数のI/Oモジュールから構成されるシステムを表現するためのクラスです。Gainer I/OモジュールやArduino I/Oボードを利用する際には、Systemオブジェクトをラップするユーティリティークラス（GainerまたはArduino）を利用してアクセスすることにより、内部構造を意識することなくアクセスできます。

\subsubsection{コンストラクタ}
\texttt{System(hostName:String, portNumber:int, modules:int, samplingInterval:int)}
...
\texttt{module(id:int)} & IOModule & 指定したIDのI/Oモジュールを返す \\
\end{tabular}
...
\subsubsection{コンストラクタ}
\texttt{IOModule(hostName:String, portNumber:int, config:Configuration, samplingInterval:int)}
...
\subsubsection{コンストラクタ}
\begin{tabular}{l}
\texttt{Gainer(hostName:String, portNumber:int, mode:int)} \\
\end{tabular}
...
\subsubsection{コンストラクタ}
\begin{tabular}{l}
\texttt{Arduino(hostName:String, portNumber:int)} \\
\end{tabular}

\subsubsection{メソッド}
\begin{tabular}{lll}
メソッド名 & 戻り値 & 説明 \\\hline
\texttt{setDigitalPinMode(pin:int, mode:int)} & void & デジタルピンのモードを設定 \\
...
\texttt{/in} & int, int, float, ... & モジュール番号、ポート番号、値1、値2…
\end{tabular}
